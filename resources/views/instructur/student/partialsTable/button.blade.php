<div class="d-flex gap-2">
	...
	@if ($user->disabled)
		<form action="{{ route('admin.student.enable', $data['uid']) }}" method="POST" id="data-enable-{{ $data['uid'] }}">
			@csrf
			@method('PUT')
	...
	@else
		<form action="{{ route('admin.student.disable', $data['uid']) }}" method="POST"
	...
	@endif
	<form action="{{ route('admin.student.destroy', $data['uid']) }}" method="POST" id="data-delete-{{ $data['uid'] }}">
		@csrf
